...
const keepAliveNames = ['ProblemListView', 'AiLabView', 'SubmissionsView', 'ProfileView']

const viewKey = computed(() => {
  if (route.name === 'problem-detail') return `problem-${route.params.id}`
  if (route.name === 'submission-detail') return `submission-${route.params.id}`
    ...
    <header class="nav">
      <div class="nav-inner">
        ...
        <nav class="links">
          ...
          <RouterLink v-if="auth.isAdmin" to="/admin" class="nav-link nav-admin" active-class="">管理后台</RouterLink>
        </nav>
        <div class="nav-right">
    ...
    <main class="main">
      <RouterView v-slot="{ Component }">
        <KeepAlive :include="keepAliveNames">
          <component :is="Component" v-if="Component" :key="viewKey" />
        </KeepAlive>
      </RouterView>
    </main>
  </div>
